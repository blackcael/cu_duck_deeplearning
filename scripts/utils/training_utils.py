...
from utils.DuckieDriveDataset import DuckieDriveDataset, get_num_datapoints
...
def loss_fn_v2(logits, truth_angle, angle_loss_weight=0.2):
    """
    Hybrid loss:
    - Cross-entropy on steering class bins.
    - SmoothL1 on differentiable expected steering angle.
    """
    truth_angle = truth_angle.float().view(-1)
    truth_class = torch.clamp(torch.round(truth_angle + 22), 0, 44).long()
...
def train(model, optimizer, training_dataloader, validation_dataloader, n_optimization_steps, log_interval, val_interval, device, angle_loss_weight = 0.2):
    # Init Variables
    training_loss = []
    val_loss = []
    ...
    while i < n_optimization_steps:
        for img, truth_angle in training_dataloader:
            ...
            logits = model(img)
            loss = loss_fn_v2(logits, truth_angle, angle_loss_weight=angle_loss_weight)
...
def training_pipelpine(
    model,
    image_size = (32, 32),
    n_minibatch_steps = 1000,
    turn_data_smoothing_window = 11,
    use_blue = False,
    log_interval = 10,
    val_interval = 10,
    angle_loss_weight = 0.2,
    model_name = None, # Option to overwrite the name (for special param configuraiton)
):
    ...
    print(f"Preparing to train {model_name}")
    ...
    batch_size = 256
    training_dataloader   = DataLoader(training_dataset,   shuffle=True, batch_size=batch_size, num_workers=8, pin_memory=True, persistent_workers=True)
    validation_dataloader = DataLoader(validation_dataset, shuffle=True, batch_size=batch_size, num_workers=8, pin_memory=True, persistent_workers=True)
    ...
    print(f"Image shape is: {sample_img.shape}")
